...
np.random.seed(0)
torch.manual_seed(0)
env.reset(seed=1010101) #4101010

# Number of episodes to train for
totalNumEpisodes = 2000
max_steps = 1000
inputDims = env.observation_space.shape[0]
outputDims = env.action_space.shape[0]
totalRewards = 0
runIndex = 10 # Set index for averaging graph
rewards_to_plot = []
...
with open(f'REINFORCE{runIndex}.json', 'w') as file:
    json.dump(rewards_export, file)

with open(f'REINFORCE{runIndex}steps.json', 'w') as file:
